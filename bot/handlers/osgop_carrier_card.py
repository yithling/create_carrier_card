...
#Если заказана карточка перевозчика с ОСГОП
@router.message(Command("osgop_carrier_card"))
...
#Скачиваем файл и создаем карточку с ОСГОП
...
async def download_file(msg: Message, bot: Bot, state: FSMContext):
    ...
    try:
        ...
        #Делаем карточку
        mode = get_mode["mode"]
        carrier_card_path = create_carrier_card.create_card(
            driver, 
            download_path, 
            mode
            )
        ...
        if carrier_card_path is not None:
            ...
            await msg.answer("Ошибка, проверьте файл с путевым или наличие ОСГОП")
